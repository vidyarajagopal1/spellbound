...
## First-Run Quest (v137–v156)

A guided onboarding flow shown to new users with no books yet, replacing a blank app with a short "quest" that builds an initial shelf, captures a first highlight, and hands off into Find Your Next Read. Full copy/interaction spec lives in `docs/quest-copy.md`, which is the single source of truth for every string — no copy in this feature was invented or reworded outside that file.

### Build (v137–v148)
Built in the order specified by quest-copy.md, each step tested live before the next: quest shell/container/state/pile component (v137), Stages 1–2 (v138), Stages 3–5's shared search-and-add pattern (v139–v142), Stage 6's three capture routes — typed, voice, OCR — and book attachment (v143–v145), the Find Your Next Read handoff with its thin-pile gate (v146–v147), and finally the trigger logic + intro screen (v148). Reuses existing components throughout: the `.book-spine` pile tile (shared with the Books tab), the shared `googleBooksSearch()` helper, and the existing voice/OCR capture paths from Add Highlight.

### Visual Polish Round 1 — Intro & Real Pile Fixes (v149–v151)
- **v149**: Intro screen hierarchy/shelf-ownership feedback — pseudo shelf de-emphasized (opacity + scale) so it reads as an example, not an already-assigned library
- **v150**: Fixed a rotation-drift bug where the intro's pseudo shelf hid spine titles — a custom overlap ratio hadn't accounted for rotation drifting a full-width bar's visual edges by `width × sin(angle)`; reverted to the app's proven default `-6px`/44px overlap ratio. Also added the first quest-specific gold theming pass, scoped to `#quest-overlay`
- **v151**: The same rotation-drift root cause recurred in the real session pile's `renderQuestPile()`; fixed by deriving the spine step from the proven ratio and adding an explicit rotation buffer to the shelf's computed height, so the shelf grows to fit the pile instead of over-compressing

### Redesign — Typography, Transitions, Bookend Treatment (v152–v153)
- **v152**: Stage-to-stage fade/rise transition (pure CSS, no JS); a Caveat-script "Step N of 6" wayfinding label and pile-counter, replacing blank/plain versions; larger question-stage headings; a paper-note card treatment (reusing the Home tab's folded-corner motif) marking the intro and FNR handoff as distinct "bookend" moments versus the numbered question stages
- **v153**: Four Find Your Next Read handoff fixes — low pile counts spelled out ("You've got one" instead of "1"); swapped button emphasis so "Add a few more" (the recommended action) is primary instead of "Go ahead anyway"; restored the "Fill in only the parts you feel strongly about" instruction (previously lost) as its own screen shown before the FNR form opens; bookend heading treatment extended to the FNR handoff. All four applied to both the five-or-more and under-five gate paths, not just one branch

### Light Theme Redesign (v154–v156)
A full restyle inspired by an outside ad's typography/contrast treatment, planned via an explicit phase-by-phase proposal the user approved/adjusted before each round.
- **v154**: Moved the FNR "fill in only the parts…" instruction into its own screen rendered inside the quest overlay itself (previously it lived inside the Wishlist-tab FNR form, showing the app's normal header/Back-to-Wishlist chrome, which broke the sense of still being inside the quest)
