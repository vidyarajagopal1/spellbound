...
async function retryWishlistDescription(id) {
  const item = wishlist.find(w => w.id === id);
  if (!item) return;
...
4. Keep variety: do not return near-identical books or more than 2 books by the same author. When the resolved reference (see above) is itself an author, exactly 2 of the 5 recommendations must be real books by that author not on the EXCLUDE LIST — not more, not fewer — and the remaining 3 must be by other authors who write in similar themes, tone, or voice to them, so every recommendation connects back to the resolved author, not just the 2 written by them. If fewer than 2 of their books survive the exclude list, fill the remaining slots with those same similar-themes/tone/voice authors instead — never leave a resolved author entirely unrepresented just because some of their books were excluded, and never exceed 2 of their own books even when more would survive the exclude list.
...
6. Each "why_it_fits" must reference at least one specific signal from the READER'S REQUEST. If nothing in the READER'S REQUEST can be cited specifically, you may instead ground "why_it_fits" in the TASTE CALIBRATION profile — but only by naming a specific book or highlight from it that the pick resonates with, never a generic statement about what the reader "tends to enjoy." The ban on generic, unsupported statements applies either way. Write it in second person per rule 12 regardless of which source you cite. When falling back to TASTE CALIBRATION across multiple recommendations, do not cite the same profile book more than twice among the 5 — see rule 14 for the full citation-variety requirement.
...
14. CITATION VARIETY: when multiple recommendations fall back to TASTE CALIBRATION per rule 6 (nothing specific in the READER'S REQUEST to cite instead), do not let one single profile book anchor every "why_it_fits" just because it's the strongest available match — this produces a set of 5 that all point back to the same one book, which defeats the purpose of giving 5 varied recommendations. Deliberately spread citations across DIFFERENT qualifying books from tiers (a)/(c) (e.g. a different highlight, aftertaste, or completed-unrated book for each pick) whenever more than one exists — the same specific book may be cited at most twice across all 5 recommendations, never more, even if it is the reader's only strongly-rated book and even if it is thematically compatible with every pick. If tiers (a)/(c) together contain only one or two real candidate books, cite each of those the minimum number of times needed to ground all 5 picks, and do not manufacture false variety — but always check first whether every finished book in (c), not just the one rated book in (a), can honestly ground a given pick before repeating (a)'s book again.
  ...
  "recommendations": [
    {
      ...
      "why_it_fits": "1–2 sentences, written directly to the reader in second person (\"you\"/\"your\"), referencing their specific request. Be specific.",
